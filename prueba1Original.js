export function mergeArrays(first) {
    let firstCopy = [] //creo una variable que sera la copia de first para guardar los valores de first ordenados en un solo array
    
    first.forEach(array => {
      array.forEach(number => {
        firstCopy.push(number)
      })
    }) //convierto la matriz en un solo array

    firstCopy.sort((a,b) => a - b)//ordenar los valores de firstCopy
    //console.log(firstCopy)//se me olvidó borrar el console.log
    return firstCopy
  }
