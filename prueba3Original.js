function decipher(message, shift) {
    ...
    const newShift = (shift % upperCaseLetters.length + upperCaseLetters.length) % 
          upperCaseLetters.length //obtengo el desplazamiento cojo el desplazamiento original 
          // y calculo el resto de la division entre la longitud del regex para no superar 
          // la longitud del regex, le sumo la longitud del regex para que el desplazamiento 
          // sea positivo
    ...
    for(let character in message){ //recorro el mensaje
      if(upperCaseLetters.includes(message[character])){ //si el caracter es una letra mayuscula
        ...
        const newIndex = (index + newShift) % upperCaseLetters.length //obtengo el nuevo indice,
        // sumo el indice original y el desplazamiento, calculo el resto de la division entre la
        ...
        const newIndex = (index + newShift) % lowerCaseLetters.length//obtengo el nuevo indice,
        // sumo el indice original y el desplazamiento, calculo el resto de la division entre la
        // longitud del regex para no superar la longitud del regex
